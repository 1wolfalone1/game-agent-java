...
public class SearchGameController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = EnumAction.SEARCH_GAME_JSP.getAction();
        try {
            String searchValue = request.getParameter("searchGame");
            GameDAO dao = new GameDAO();
            ArrayList<GameDTO> list = null;
            if (searchValue.matches("(?=.*[\\S]).*")) {
                list = dao.filterGameByName(searchValue);
                if(list.isEmpty()){
                    list = null;
                }
            }
